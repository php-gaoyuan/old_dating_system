...
//定义写操作
  ...
  $dbo->exeUpdate($sql);
	...
	$scrip_content=$ea_langpackage->ea_you_assigned_to.'<a href="home2.0.php?h='.$user_id.'&app=event_space&event_id='.$event_id.'" target="_blank">'.$event_name.'</a>'.$ea_langpackage->ea_organizing_people;
	$is_success=api_proxy('scrip_send',$ea_langpackage->ea_system_sends,$title,$scrip_content,$member_id,0);
	if($is_success){
		api_proxy("message_set",$member_id,$ea_langpackage->ea_num_notice,"modules2.0.php?app=msg_notice",0,11,"remind");
	}
