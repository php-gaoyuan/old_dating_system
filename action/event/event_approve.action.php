...
//通知
	...
	$scrip_content=$ea_langpackage->ea_you_join.'<a href="home2.0.php?h='.$user_id.'&app=event_space&event_id='.$event_id.'" target="_blank">'.$event_info['title'].'</a>'.$ea_langpackage->ea_activity_app_by;
	$is_success=api_proxy('scrip_send',$ea_langpackage->ea_system_sends,$title,$scrip_content,$member_id,0);
	if($is_success){
		api_proxy("message_set",$member_id,$ea_langpackage->ea_num_notice,"modules2.0.php?app=msg_notice",0,1,"remind");
	}

//纪录新鲜事
	$title=$ea_langpackage->ea_add_activity.'<a href="home2.0.php?h='.$user_id.'&app=event_space&event_id='.$event_id.'" target="_blank">'.$event_info['title'].'</a>';
	$content=$event_info['detail'];
	$is_suc=api_proxy("message_set",0,$title,$content,0,11,$member_id);
	
  $jump="modules2.0.php?app=event_member_manager&event_id=$event_id";
