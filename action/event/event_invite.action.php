...
if(!empty($pals_id)){
	...
	$scrip_content=$user_name.$ea_langpackage->ea_invite_participate.'<a href="home2.0.php?h='.$user_id.'&app=event_space&event_id='.$event_id.'" target="_blank">'.$event_name.'</a>'.$ea_langpackage->ea_activity.'<br />'.$ea_langpackage->ea_you_can.'<a href="javascript:void(0)" onclick="join_event('.$event_id.')">'.$ea_langpackage->ea_accept_invite.'</a>'.$ea_langpackage->ea_or_view.'<a href="home2.0.php?h='.$user_id.'&app=event_space&event_id='.$event_id.'" target="_blank">'.$ea_langpackage->ea_event_details.'</a>';
	...
	foreach($pals_id as $key => $value){
		...
		if($is_success){
			api_proxy("message_set",$value,$ea_langpackage->ea_num_notice,"modules2.0.php?app=msg_notice",0,1,"remind");
		}
	}
}
	$jump="modules2.0.php?app=event_invite&event_id=$event_id";
