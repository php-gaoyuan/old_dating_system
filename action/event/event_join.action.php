...
if($event_row['verify']==0 || $is_invite['to_user_id']){
	...
	//纪录新鲜事
	$title=$ea_langpackage->ea_add_activity.'<a href="home2.0.php?h='.$user_id.'&app=event_space&event_id='.$event_id.'" target="_blank">'.$event_row['title'].'</a>';
	$content='<a href="home2.0.php?h='.$user_id.'&app=event_space&event_id='.$event_id.'" target="_blank">'.$event_row['title'].'</a>';
	...
	if($dbo->exeUpdate($sql)){
		...
		foreach($e_manager as $val){
			api_proxy("message_set",$val['user_id'],$ea_langpackage->ea_num_people_request_join,"modules2.0.php?app=event_member_manager&event_id=".$event_id,0,11,"remind");//提醒机制
		}
