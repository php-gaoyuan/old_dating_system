...
//更新用户表
	...
	//纪录新鲜事
	$title=$g_langpackage->g_joined_group.'<a href="home2.0.php?h='.$user_id.'&app=group_space&group_id='.$group_id.'" target="_blank">'.$group_info['group_name'].'</a>';
	$content=$group_info['group_resume'];
	$is_suc=api_proxy("message_set",0,$title,$content,6,1,$userid);

	if(isset($type)&&$type=="remind"){
		$jump="modules2.0.php?app=remind_group&group_id=".$group_id;
		action_return(1,"",$jump);
	}else{
		$jump="modules2.0.php?app=group_member_manager&group_id=".$group_id;
		action_return(1,"",$jump);
	}
