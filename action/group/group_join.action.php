...
if($group_row['group_join_type']==0){
	...
	//纪录新鲜事
	$title=$g_langpackage->g_joined_group.'<a href="home2.0.php?h='.$user_id.'&app=group_space&group_id='.$group_id.'" target="_blank">'.$group_name.'</a>';
	$content='<a href="home2.0.php?h='.$user_id.'&app=group_space&group_id='.$group_id.'" target="_blank">'.$group_name.'</a>';
...
if($group_row['group_join_type']==1){
	...
	foreach($g_manager as $val){
		api_proxy("message_set",$val['user_id'],$g_langpackage->g_remind,"modules2.0.php?app=group_member_manager&group_id=".$group_id,0,10,"remind");//提醒机制
	}
