...
$msg_title = short_check(get_argp("msg_title"));
...
dbtarget('w', $dbServs);
$uinfo = $dbo->getRow("select golds, user_sex, user_group, integral, email_num, email_time from wy_users where user_id='$user_id'");
...
if(($uinfo['user_group']=='base' || $uinfo['user_group']==1) && $uinfo['user_sex'] == 1){
	if($uinfo['mail_num']>2){
		if ($uinfo['golds'] <1) {
			...
			echo "<script>top.location.href='http://www.enjoylover.com/main2.0.php?app=user_pay';</script>";exit;
...
if ($uinfo['user_group'] == 1 && $uinfo['user_sex'] == 1) {
...
if ($users_row) {
	...
	if ($touser_id == $user_id) {
		...
		action_return(0, $m_langpackage->m_no_mys, "modules2.0.php?app=msg_creator");

	}

} else {

	action_return(0, $m_langpackage->m_one_err, "modules2.0.php?app=msg_creator");

}


$score = 1;
if (($user_self['golds'] - $score) >= 0 && $user_self['user_sex'] == 1 && $user_self['user_group'] == 1) {
	//echo $score;exit;
...
/*else{
action_return(0,$m_langpackage->m_cread_put,"modules2.0.php?app=user_pay");
}*/
...
if ($dbo->exeUpdate($sql)) {
	...
	api_proxy("message_set", $touser_id, $m_langpackage->m_remind, "modules2.0.php?app=msg_minbox", 0, 5, "remind");

	if (get_argp('nw') == "2") {

		action_return(1, '', "modules2.0.php?app=msg_rpshow&hhid=" . $hh_id);
