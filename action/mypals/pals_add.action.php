...
//判断自己是否存在对方朋友圈中
	...
	if(empty($user_mypals)){
		...
		if($is_success){
			...
			api_proxy("message_set",$mreq_touser,"{num}".$mp_langpackage->mp_a_notice,"modules2.0.php?app=msg_notice",0,1,"remind");
...
if($touser_pals_limit==1){
 ...
 if($dbo->exeUpdate($sql)){
 	api_proxy("message_set",$mreq_touser,$mp_langpackage->mp_remind,"modules2.0.php?app=mypals_request",0,3,"remind");//提醒机制
