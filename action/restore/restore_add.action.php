...
//变量定义区
	...
	$to_userid=intval(get_argp('to_userid'));
	$mod_id=intval(get_argp('mod_id'));
	$type_id=intval(get_argp('type_id'));
	$hidden_talk=intval(get_argp('hidden_talk'));
	$is_true=0;
...
//数据表定义
  ...
  $dbo=new dbex();
		...
		switch($type_id){
			...
			$link="home2.0.php?h=$host_id&app=blog&id=$mod_id";
			break;
			case "1":
			$group_info=array();
			...
			$link="home2.0.php?h=$host_id&app=group_sub_show&subject_id=$mod_id&group_id=$group_id";
			break;
			case "2":
			$t_table=$t_album;
			...
			$link="home2.0.php?h=$host_id&app=photo_list&album_id=$mod_id";
			break;
			case "3":
			$photo_info=array();
			$t_table=$t_photo;
			...
			$link="home2.0.php?h=$host_id&app=photo&photo_id=$mod_id&album_id=".$photo_info['album_id'];
			break;
			case "4":
			$t_table=$t_poll;
			...
			$link="home2.0.php?h=$host_id&app=poll&p_id=$mod_id";
			break;
			case "5":
			$t_table=$t_share;
			...
			$link="home2.0.php?h=$host_id&app=share_list&remind=1&mod=$mod_id";
			break;
			case "6":
			$t_table=$t_mood;
			...
			$link="home2.0.php?h=$host_id&app=mood_more&remind=1&mod=$mod_id";
			break;
			case "7":
			$t_table=$t_event;
			...
			$link="home2.0.php?h=$host_id&app=event_space&event_id=$mod_id";
			break;
...
if($host_id!=$visitor_id){
	...
	if($dbo->exeUpdate($sql)){
		...
		$remind_content='{num}回复了您的心情';
		if($langPackagePara == 'en'){
			$remind_content='{num}Replied to your mood';
		}
	...
	if($last_id){
		...
		echo '<dl class="child_box1_lf" >
            <dt>
            <a target="_blank" class="name_box1_lf" href="home2.0.php?h='.$visitor_id.'"><img src="'.$visitor_ico.'" width="38" height="38" alt="admin9999"></a>
            </dt>
            <dd>
            <p><a class="name_box1_lq">'.filt_word($visitor_name).'</a></p> 
            <div class="somethingnew_tr">'.get_face2($content).'</div>
            <div class="info_box1_lf">
            <span class="send_time" >'.date("Y-m-d H:i:s",time()).'</span>
