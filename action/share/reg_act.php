...
//检测邀请码
	...
	if(get_argp("tuid"))
	{
		$sql="insert into $t_users (user_name,user_pws,user_sex,user_email,user_add_time,user_ico,user_group,invite_from_uid,is_pass,lastlogin_datetime,birth_year , birth_month , birth_day ,login_ip,zhuce_ip,tuid ) values('$user_name','$user_pws',$user_sex,'$user_email','".constant('NOWTIME')."','$user_ico','1',$invite_fromuid,$is_pass,'".constant('NOWTIME')."','','','','$_SERVER[REMOTE_ADDR]','$_SERVER[REMOTE_ADDR]',".get_argp("tuid").")";
	}
	else if(get_argp("uid"))
	{
		$sql="insert into $t_users (user_name,user_pws,user_sex,user_email,user_add_time,user_ico,user_group,invite_from_uid,is_pass,lastlogin_datetime,birth_year , birth_month , birth_day ,login_ip,zhuce_ip,uid ) values('$user_name','$user_pws',$user_sex,'$user_email','".constant('NOWTIME')."','$user_ico','1',$invite_fromuid,$is_pass,'".constant('NOWTIME')."','','','','$_SERVER[REMOTE_ADDR]','$_SERVER[REMOTE_ADDR]',".get_argp("uid").")";
	}
	else
	{
		$sql="insert into $t_users (user_name,user_pws,user_sex,user_email,user_add_time,user_ico,user_group,invite_from_uid,is_pass,lastlogin_datetime,birth_year , birth_month , birth_day ,login_ip,zhuce_ip ) values('$user_name','$user_pws',$user_sex,'$user_email','".constant('NOWTIME')."','$user_ico','1',$invite_fromuid,$is_pass,'".constant('NOWTIME')."','','','','$_SERVER[REMOTE_ADDR]','$_SERVER[REMOTE_ADDR]')";
	}
	...
	if($user_sex==0)
	{
		...
		$dbo->exeUpdate($sql);
		action_return(1,$re_langpackage->re_gock_u,'index.php');
		exit();
	}
	$now_time=time();
...
<?php
	//需要激活时的操作
		...
		$mailbody = "尊敬的".$user_name."：<br />您好：".'<br />'."您在".$siteName."上注册了新用户，请点击下面的链接激活您的账户<br /><a href='".$siteDomain."modules2.0.php?app=user_activation&user_email=".$user_email."&activation_code=".$activation_code."'>href='".$siteDomain."modules2.0.php?app=user_activation&user_email=".$user_email."&activation_code=".$activation_code."</a>";
...
?>
	<script type='text/javascript'>
		location.href='modules2.0.php?app=user_activate_succ&user_email=<?php echo $user_email ?>';
	</script>
<?php
	}
}
