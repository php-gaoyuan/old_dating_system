...
$money=$stamps*2;
if($money > $golds){
	echo "<script>alert('".$er_langpackage->er_slerror."');location.href='/modules2.0.php?app=user_pay';</script>";
	exit();
}
...
if($dbo->exeUpdate($sql)){
    echo "<script>alert('".$er_langpackage->er_slsucess."');location.href='/modules2.0.php?app=user_stamps';</script>";
	exit();   
}else{
    echo "<script>alert('".$er_langpackage->er_sler."');location.href='/modules2.0.php?app=user_stamps';</script>";
	exit();    
}
