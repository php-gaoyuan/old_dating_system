...
if($user_row){
	...
	$mailbody = $u_langpackage->u_dear.$user_row['user_name'].$u_langpackage->u_user.'：<br />'.$u_langpackage->u_hello.'<br />'.$u_langpackage->u_you_as.$siteName.$u_langpackage->u_click_link_amended.'<br />
							<a href="'.$siteDomain.'modules2.0.php?app=user_pw_reset&uid='.$user_row['user_id'].'&email='.$email.'&code='.$code.'">'.$siteDomain.'modules2.0.php?app=user_pw_reset&uid='.$user_row['user_id'].'&email='.$email.'&code='.$code.'</a>';
