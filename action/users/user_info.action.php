<?php
	...
	//$info = get_argp('info');
	$country = short_check(get_argp('country'));
	$income = get_argp('income');
	$height = get_argp('height');
	$weight = short_check(get_argp('weight'));
	$waimao = get_argp('waimao');
	$gerenjieshao = get_argp('gerenjieshao');
	$sexual = get_argp('sexual');
	//echo $country;exit;
	//表声明区
...
//更新users表
	$sql = "update $t_users set `height`='$height',`income`='$income',country='$country',birth_province='$birth_province',birth_city='$birth_city',reside_province='$birth_province',reside_city='$birth_city',birth_year='$birth_year',birth_month='$birth_month',birth_day='$birth_day',waimao='$waimao',sexual='$sexual',weight='$weight',gerenjieshao='$gerenjieshao' 
...
//更新online表
	...
	//回应信息
	action_return(1,"","modules2.0.php?app=user_info&single=$is_finish&user_id=$user_id");
?>
