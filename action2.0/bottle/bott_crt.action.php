<?php
  ...
  //判断是否为普通用户
  $user_info = $dbo->getRow("select user_name,user_group from $t_users where user_id=$to_user_id");
/*
  if($user_info['user_group']=='base' || $user_info['user_group']=='1'){
        action_return(1,'漂流瓶功能暂不对普通用户开放，请升级高级会员','modules.php?app=user_pay');
        exit();
  }
  */
  //获取网站当前会员数
  ...
  if($dbo->exeUpdate($sql)){
  	  if($sink==0){
  	  	  action_return(1,str_replace('{range}',rand(1,10000),$b_langpackage->b_range),'-1');
          //echo "<script type='text/javascript'>alert('您的漂流瓶漂到了');window.close();</script>";
  	  }else{
  	  	  action_return(1,$b_langpackage->b_range2,'-1');
  	  }
  }else{
  	  action_return(0,$s_langpackage->b_fail,'-1');
  }
?>
