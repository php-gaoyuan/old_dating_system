<?php
  ...
  $dbo = new dbex;
  	...
  	//读写分离定义函数
  	dbtarget('w',$dbServs);
	$uinfo=$dbo->getRow("select user_sex,user_group,integral,email_num,email_time from wy_users where user_id='$user_id'");

	//普通会员（免费发三封）
	/*if(($uinfo['user_group']=='base' || $uinfo['user_group']==1) && $uinfo['user_sex'] == 1){
		if($uinfo['email_num']>2){
