...
if($nowtime>=$endtime && $user_info['user_sex'] =='1' )
 {  
  ...
  if($users_row){
	  ...
	  {
		  action_return(0,$m_langpackage->m_no_mys,"modules2.0.php?app=msg_creator".$toidUrlStr);
	  }
  }else{
		action_return(0,$m_langpackage->m_one_err,"modules2.0.php?app=msg_creator".$toidUrlStr);
  }
  ...
  if (!empty($_FILES["mypictures"]["name"])) { //提取文件域内容名称，并判断
	...
	if($dbo ->exeUpdate($sql)){
		...
		if(get_argp('nw')=="2"){
			action_return(1,'',"modules2.0.php?app=msg_moutbox".$toidUrlStr);
		}else{
			action_return(1,'',"modules2.0.php?app=msg_moutbox".$toidUrlStr);
		}
	}else{
