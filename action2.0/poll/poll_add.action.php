...
//定义写操作
  ...
  foreach($cho_array as $val){
	...
	if($makefeed==1){
		//纪录新鲜事
		$title=$pol_langpackage->pol_launch_vote.'<a href="home2.0.php?h='.$user_id.'&app=poll&p_id='.$last_pid.'" target="_blank">'.$subject.'</a>';
		$content='<a href="home2.0.php?h='.$user_id.'&app=poll&p_id='.$last_pid.'" target="_blank">'.$subject.'</a>';
