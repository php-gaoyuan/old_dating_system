...
//echo time();
//消息机制写操作
function message_set_app($id='',$title,$content='',$type=0,$is_mod=0,$mod='affair',$uid=null){
	//echo time();
	...
	switch ($mod){
		...
		$type=intval($is_mod);
		return message_set_remind($touid,$title,$link,$type,$is_focus,$uid);
		break;
		default:
		$id=intval($id);
...
function getuserinfo($uid){
	...
	$sql="select * from wy_users where user_id='$uid'";
		//echo  $sql;
		//dbtarget('r',$dbServs);
		$dbo=new dbex;
		dbplugin('r');
...
function message_set_remind($touid,$content,$link,$type,$is_focus,$uid){
	$uid=$uid;
