...
class Fhtpay{
	...
	public function fhtpay($order) {
		// 验证信用卡号
		$reg = '/^\d{15,19}$/';
		if(!preg_match($reg,$order["card_number"])){
			//return array("errno"=>1,"msg"=>"卡号错误");
		}

		$exp_date = $order["exp_year"]."-".$order["exp_month"];
		// 验证过期时间
		if(!preg_match('/^\d{4}-\d{2}$/',$exp_date)){
			return array("errno"=>1,"msg"=>"有效期错误");
		}

		// 验证CVV
		if(!preg_match('/^\d{3,4}$/',$order["cvv"])){
			return array("errno"=>1,"msg"=>"CVV错误");
		}
		...
		// 支付成功
		if($code == '00'){
			return array("errno"=>0,"data"=>$obj);
		}else{
			return array("errno"=>1,"msg"=>$obj->respMsg);
		}
	}
	// 获取用户真实IP
	function getIP() {
