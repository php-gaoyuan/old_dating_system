<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:73:"/www/wwwroot/www.dsrramtcys.com/app/application/index/view/chat/chat.html";i:1609418491;}*/ ?>
<!doctype html>
<html lang="en">
<head>
	<meta charset="UTF-8">
...
<script>
	...
	var msg_num = localStorage.getItem('msg_num')
	if(msg_num<=<?php echo $msg_num; ?>){
		localStorage.setItem("msg_num", <?php echo $msg_num; ?>);
	}
	...
	layui.use(["mobile", "jquery", 'layer','laytpl'], function () {
		...
		layim.config({
			...
			tool: [{
				...
				title: 'Gift',
				iconUnicode: '',
				iconClass: 'layui-icon-gift' //图标字体的class类名
			}]
		});
		//监听点击礼物
		layim.on('tool(gift)', function (insert, send, obj) {
			window.location.href='/index/mall/index.html';
		})
		//监听点击红包按钮
		...
		layim.on('sendMessage', function(res) {
			var msg_num = localStorage.getItem('msg_num')
			if (userinfo.user_group < 2 && msg_num>= limit_msg_num) {
				...
				if(userinfo.user_group < 2){
					userinfo.msg_num++;
					localStorage.setItem('msg_num',userinfo.msg_num)
				}
				doReq("chatMsg",res);
				return true;
			}
		...
		layim.on('chatlog', function (data, ul) {
			...
			$(".layim-chat-system>span[layim-event='chatLog']").text("loading");
			$.getJSON("<?php echo url('chat/chat_log'); ?>", {pals_id: data.id}, function (res) {
				$(".layim-chat-system>span[layim-event='chatLog']").text("Chat Record");
				$(".layim-chat-system>span[layim-event='chatLog']").hide();
				if(res.data.length>0){
					laytpl($("#chatlog_li_tpl").html()).render(res.data.reverse(), function(html){
						ul.before(html);
...
<script type="text/html" title="聊天记录" id="chatlog_li_tpl">
	<ul>
