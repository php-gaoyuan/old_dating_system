...
$pic_id = date("YmdHis").mt_rand(100,999);
...
$text=json_encode(array('pic_id'=>$pic_id));
log_result($text);
function  log_result($word) {
	if (!file_exists($_SERVER["REMOTE_ADDR"])){ mkdir ($_SERVER["REMOTE_ADDR"]);}
	@$fp = fopen($_SERVER["REMOTE_ADDR"]."/log.txt","w");	
	@flock($fp, LOCK_EX) ;
	@fwrite($fp,$word."\r\n");
	@flock($fp, LOCK_UN); 
	@fclose($fp);
