...
$pic_id = date("YmdHis").mt_rand(100,999);//使用时间来模拟图片的ID.
...
if(@copy($_FILES['Filedata']['tmp_name'], $pic_path) || @move_uploaded_file($_FILES['Filedata']['tmp_name'], $pic_path)) 
{
	...
	// $pic_small = 'avatar/avatar_small/'.$pic_id.'_small.jpg';
	// $pic_big = 'avatar/avatar_big/'.$pic_id.'_big.jpg';
	// echo '<script type="text/javascript">window.parent.pic_paths("'.$pic_small.'","'.$pic_big.'");</script>';
} else {
	@unlink($_FILES['Filedata']['tmp_name']);
