...
function page_show($mod,$page_num,$page_total){
	if($mod==0){
		return pagesbar($page_num,$page_total,'page');
	}
}
...
function pagesbar($page_num,$page_total,$page)
{
	...
	{
		$prePageUrl=$get_para.$sign.$page.'=1';
	}
	...
	{
		$nextPageUrl=$get_para.$sign.$page.'='.($page_total);
	}
	
	
	$pagearr=array();
	
	$pagearr['up']['url']=$prePageUrl;
	$pagearr['up']['tag']=$page_up;
	$pagearr['down']['url']=$nextPageUrl;
	$pagearr['down']['tag']=$page_down;
	
		$pagearr['total_page']=$page_total;
	$pagearr['curr_page']=$page_num;
	$pagearr['pageurl']=$get_para.$sign.$page.'=';;
