...
function get_face2($str){
	...
	foreach($match_none as $rs){
		...
		$rep_str[] = '<img class="face_em" src="'.$siteDomain.'skin/'.$skinUrl.'/images/qq/'.$arr[0].'.gif" />';
	}
...
function get_face($str){
	...
	foreach($match_none as $rs){
		...
		//$rep_str[] = '<img class="face_em" src="'.$siteDomain.'skin/'.$skinUrl.'/images/em/'.$arr[0].'.gif" />';
	}
	return str_replace($match_array[0],$rep_str,$str);
}
