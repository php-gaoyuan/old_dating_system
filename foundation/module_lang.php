...
echo "<select name='change_lp' onchange=document.getElementById('langPackagePara').value=this.value;>";
$res=opendir("../langpackage");
while($lp_dir=readdir($res)){
	if(!preg_match("/^\./",$lp_dir)){
		$l_selected='';
...
echo "</select><input type='hidden' name='langPackagePara' id='langPackagePara' value='".$def_lp."' />";	
}
?>