...
if (strpos($agent, "comFront") || strpos($agent, "iPhone") || strpos($agent, "MIDP-2.0") || strpos($agent, "Opera Mini") || strpos($agent, "UCWEB") || strpos($agent, "Android") || strpos($agent, "Windows CE") || strpos($agent, "SymbianOS")) {
    ...
    //echo "<pre>";var_dump(strpos($_SERVER['SCRIPT_NAME'],'member')===false);exit;
    if(strpos($_SERVER['SCRIPT_NAME'],'payment')===false && strpos($_SERVER['SCRIPT_NAME'],'member')===false){
        $url="{$protocol}m.{$rootUrl}/index/main/index.html";
        header("Location:{$url}");
        exit;
    }
}
