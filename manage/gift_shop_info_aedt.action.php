...
if(get_argp('motion')=='add'){
	...
	if(empty($info_giftname)){
		echo "<script type='text/javascript'>alert('礼品名称不能为空');window.history.go(-1);</script>";exit;
	}
	
	if(empty($_FILES['info_patch']['tmp_name'][0])){
		echo "<script type='text/javascript'>alert('礼品图片不能为空');window.history.go(-1);</script>";exit;
	}
	// if(empty($desc)){
...
$s_icos = substr($s_icos, 0,-1);
	...
	$sql="insert into gift_shop (typeid,valentine,giftname,money,yuanpatch,patch,`desc`,ordersum,edittime) values('$type','$valentine','$info_giftname','$money','$fileSrcStrs','$s_icos','$desc','$sort',".time().")";
