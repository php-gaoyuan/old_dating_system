...
foreach($zongjis as $rs){
	if($rs['type']=='1'){//给自己充值金币	
		$cc=$rs['funds']*0.1;
	}else if($rs['type']=='2' ){//会员升级提成
		$cc=$rs['funds']*0.2;
	}else if($rs['type']=='4'  || $rs['type']=='3'){//站内信费用3or送礼物提成4
		$cc=$rs['funds']*0.2;
...
// if($_GET['touname']){
// 	$tuid=get_session('wz_userid');
// 	$p_name=$_GET['touname'];
// 	$user_group=$_GET['user_group'];
// 	if(!empty($p_name) && empty($user_group)){
// 		$sql="select * from wy_balance as b left join wy_users as u on b.touid=u.user_id where u.user_name='$p_name' and b.state='2' and b.funds !=0 and tuid='$tuid' order by id desc";
// 	}else if(empty($p_name) && !empty($user_group)){
// 		$sql="select * from wy_balance as b left join wy_users as u on b.touid=u.user_id where u.user_group='$user_group' and b.state='2' and b.funds !=0 and tuid='$tuid' order by id desc";
// 	}else if(!empty($p_name) && !empty($user_group)){
// 		$sql="select * from wy_balance as b left join wy_users as u on b.touid=u.user_id where u.user_name='$p_name' and u.user_group='$user_group' and b.state='2' and b.funds !=0 and tuid='$tuid' order by id desc";
// 	}
// }
