...
//引入语言包
  ...
  /*
    ...
    //取出全部会员
	*/
    $sql=" select * from (select $t_mood.mood,$t_users.user_id,$t_users.user_name,$t_users.user_sex,$t_users.birth_year,$t_users.user_ico,$t_users.user_group from  $t_users  left join $t_mood on $t_users.user_id=$t_mood.user_id order by $t_mood.add_time desc) as tmp GROUP BY tmp.user_id order by rand() limit 6";
