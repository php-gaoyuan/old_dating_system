...
?><?php
	...
	if($is_self=='Y'){
		...
		$guide_txt=$a_langpackage->a_no_alb.",<a href='modules2.0.php?app=album_edit'>".$a_langpackage->a_crt_alb."</a>,<a href='modules2.0.php?app=album_friend'>".$a_langpackage->a_fri_alb."</a>";
		$button="";
	}else{
...
<div class="create_button"><a href="modules2.0.php?app=photo_upload"><?php echo $a_langpackage->a_upload;?></a></div>
<?php }?>
...
<div class="album_holder">
	...
	<?php $is_pri=check_pri($val['user_id'],$val['privacy']);?>
	  <dl class="list_album" onmouseover="this.className += ' list_album_active';" onmouseout="this.className='list_album';" <?php if($is_self=='Y'){?>title="<?php echo $a_langpackage->a_tip_pri;?>" onmouseDown="menu_pop_show(event,this);" id='<?php echo $t_album;?>:<?php echo $val['album_id'];?>:<?php echo $val["privacy"];?>'<?php }?>>
		  <dt><a href=<?php echo $is_pri ? "modules2.0.php?app=photo_list&album_id=".$val['album_id']."&user_id=".$val['user_id'] : "javascript:void(0)";?>><img onerror="parent.pic_error(this)" src=<?php echo $is_pri ? $val['album_skin'] : "skin/$skinUrl/images/errorpage.gif";?>></a></dt>
		  <dd><strong><a href="<?php echo $is_pri ? "modules2.0.php?app=photo_list&album_id=".$val['album_id']."&user_id=".$val['user_id'] : "javascript:void(0)";?>"><?php echo filt_word($val['album_name']);?></a></strong></dd>
		  ...
		  <?php if($is_self=='Y'){?>
		  <dd class="album_conf">
				<a class="album_edit" href='modules2.0.php?app=album_edit&album_id=<?php echo $val['album_id'];?>'><?php echo $a_langpackage->a_edit;?></a>
