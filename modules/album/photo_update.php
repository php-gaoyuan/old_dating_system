...
if($fs){
	...
	if($album_row['privacy']==''){
		...
		foreach($fs as $val){
			...
			$show_limit++;
			$content.='<a href="home2.0.php?h='.$user_id.'&app=photo&photo_id='.$val['photo_id'].'&album_id='.$album_id.'" target="_blank">'
		...
		$title=$a_langpackage->a_in_album.'<a href="home2.0.php?h='.$user_id.'&app=photo_list&album_id='.$album_id.'" target="_blank">'.$album_row['album_name'].'</a>'.$a_langpackage->a_upload_new_photo;
...
<!--<div class="guide_info">
	<script>
	window.location.href='modules2.0.php?app=album';
	</script>
</div>-->
<?php }?>
</body>
