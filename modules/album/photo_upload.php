...
?><?php
	...
	if($album_id==''){
		...
		//print_r($album_num);exit;
		if(!$album_num['a_num']){
			echo '<script type="text/javascript">window.location.href="modules2.0.php?app=album_edit";</script>';
		}
	}
	
	$sql="select count(*) as num from $t_photo where user_id=$user_id";
	$photo_num = $dbo->getRow($sql);
...
<form action="do.php?act=photo_upl" method="post" enctype="multipart/form-data" onsubmit="return check_form();">
	...
	<tr><th><?php echo $a_langpackage->a_cho;?>：</th><td><?php album_name($album_rs,$album_id);?></td><td></td></tr>
...
function getKeepupUrl(){
	var album_id=document.getElementById('album_name').value;
  return "modules2.0.php?app=photo_upload&album_id="+album_id;
}

function getSuccUrl(){
	var album_id=document.getElementById('album_name').value;
	return "modules2.0.php?app=photo_update&album_id="+album_id;
}
