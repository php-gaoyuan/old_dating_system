...
<body id="iframecontent">
	<div class="create_button"><a href="modules2.0.php?app=blog_edit"><?php echo $b_langpackage->b_creat;?></a></div><h2 id="page_title" class="app_blog"><?php echo $b_langpackage->b_blog;?></h2>

	<div class="tabs">
		<ul>
	  	<li><a href="modules2.0.php?app=blog_list" hidefocus="true"><?php echo $b_langpackage->b_mine;?></a></li>
	  	<li class="active"><a href="modules2.0.php?app=blog_friend" hidefocus="true"><?php echo $b_langpackage->b_friend;?></a></li>
	  </ul>
	</div>
	<?php foreach($blog_rs as $rs){?>
	<?php $is_pri=check_pri($rs["user_id"],$rs["privacy"]);?>
	<dl class="log_list friend">
		<div class="avatar"><a href="home2.0.php?h=<?php echo $rs["user_id"];?>" target="_blank" title="<?php echo $rf_langpackage->rf_v_home;?>"><img src="<?php echo $rs["user_ico"];?>" /></a></div>
		<dt>
			<strong><a href='<?php echo $is_pri ? "modules2.0.php?app=blog&id=".$rs['log_id']."&is_friend=1":"javascript:void(0)";?>'><?php echo $is_pri ? filt_word($rs["log_title"]): $b_langpackage->b_limit_blog;?></a></strong>
			<?php if(!$is_pri){?>
			<img src='skin/<?php echo $skinUrl;?>/images/user_privacye.gif' />
			<?php }?>
			<br /><span><?php echo $b_langpackage->b_sort;?>：<?php echo get_blog_sort(filt_word($rs['log_sort_name']));?></span><span><a href="home2.0.php?h=<?php echo $rs["user_id"];?>" target="_blank"><?php echo filt_word($rs['user_name']);?></a></span><span><?php echo $rs["add_time"];?></span></dt>
