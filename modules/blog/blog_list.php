...
<?php if($is_self=='Y'){?>
<div class="create_button"><a href="modules2.0.php?app=blog_edit"><?php echo $b_langpackage->b_creat;?></a></div>
<?php }?>
<h2 class="app_blog"><?php echo $str_title;?></h2>
<?php if($is_self=='Y'){?>
<div class="tabs">
	<ul>
	  <li class="active"><a href="modules2.0.php?app=blog_list" hidefocus="true"><?php echo $b_langpackage->b_mine;?></a></li>
	  <li><a href="modules2.0.php?app=blog_friend" hidefocus="true"><?php echo $b_langpackage->b_friend;?></a></li>
  </ul>
</div>
<?php }?>
...
<dl class="log_list" <?php if($is_self=='Y'){?>title="<?php echo $b_langpackage->b_right_set_competence;?>" onmouseDown="menu_pop_show(event,this);" id='<?php echo $t_blog;?>:<?php echo $rs['log_id'];?>:<?php echo $rs["privacy"];?>'<?php }?>>
	<dt>
		<strong><a href='<?php echo $is_pri ? rewrite_fun("modules2.0.php?app=blog&id=".$rs['log_id'].$url_userid):"javascript:void(0)";?>'><?php echo $is_pri ? filt_word($rs["log_title"]): $b_langpackage->b_limit_blog;?></a></strong>
		<?php if(!$is_pri){?>
		<img src='skin/<?php echo $skinUrl;?>/images/user_privacye.gif' />
		<?php }?>
		<br /><span><?php echo $b_langpackage->b_sort;?>：<a href="modules2.0.php?app=blog_list&sort_id=<?php echo $rs['log_sort'];?>&sort_name=<?php echo urlencode($rs['log_sort_name']);?><?php echo $url_userid;?>" title="<?php echo $b_langpackage->b_same_sort;?>"><?php echo empty($rs['log_sort_name']) ? $b_langpackage->b_default_sort :filt_word($rs['log_sort_name']);?></a></span><span><?php echo $rs["add_time"];?></span>
	</dt>
