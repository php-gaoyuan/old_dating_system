              ...
              if($type==1){//发件箱
                ...
                if($msg_row['state']=="0"){
                   ...
                   $reButUrl=$reTurnUrl;
                    ...
                    $hh_id=$_GET['hhid'];
					if($hh_id==''){
						header("location:modules.php?app=msg_rpshow2&id=".$_GET['id']."&t=".$type);
						exit;
					}
                    $sql="select * from $t_msg_inbox where hh_id='$hh_id' order by mess_id asc";
                    $msg_row = $dbo ->getRs($sql);
                    if(!$msg_row){header("location:".$siteDomain."modules.php?app=msg_minbox");}
					//查询对方信息
                    ...
                    $reButUrl="modules.php?app=msg_creator&2id=$from_user_id&rt=".urlencode($mess_title)."&mesid=".$you_info;
...
function join_event(event_id,fellow,template){
	$.post("do.php?act=event_join&event_id="+event_id+"&fellow="+fellow+"template="+template,function(c){parent.Dialog.alert(c);location.href="modules.php?app=event_space&event_id="+event_id});
}
...
<body id="iframecontent">
	...
	<div class="mess_main">
		...
		<div class="mess_xunhuan">
			<div class="mess_xh_tit"><span><a href="do.php?act=msg_del&t=all&hhid=<?php echo $hh_id;?>"><?php echo $m_langpackage->m_qbsc;?></a></span><span><a href="modules.php?app=msg_minbox"><?php echo $m_langpackage->m_fanhuiliebiao;?></a></span></div>
		</div>
