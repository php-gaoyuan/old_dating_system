    ...
    <head>
        ...
        <script type='text/javascript'>
            ...
            function mypals_add(other_id) {
                ...
                mypals_add.getInfo("do.php", "get", "app", "act=add_mypals&other_id=" + other_id,
                function(c) {
        ...
        <style>
            ...
            .avatar img{width:80px;height:80px} .details{width:400px;margin-left:20px;}
            .content{height:30px;line-height:30px;position:relative} .content img{cursor:pointer;}
            .add_f{display:block;width:90px;font-size:12px;color:#385679;cursor:pointer;position:absolute;right:0;top:0}
            .add_f:hover {color:#ff0000;} .add_f img{position:relative;top:5px} .content_ch1{width:90px;height:20px;background:url("skin/default/jooyea/images/ch.jpg")
            no-repeat;} .content_ch1 a{width:22px;height:20px;display:block;float:left;cursor:pointer;}
            .lpages{margin:10px 0 20px;} .content_mood{background-color:#EEEFF3;height:30px;width:510px;text-indent:5px;}
            ...
            .content_mood:hover #fanyi_mood{display:block;} .rs_mood{width:405px;float:left;height:30px;overflow:hidden}
        </style>
    </head>
    ...
    <body id="iframecontent">
        <div class="create_button">
            <a href="modules.php?app=mypals_search">
                <?php echo $mp_langpackage->mp_re_search;?>
            </a>
        </div>
        <h2 class="app_friend">
            <?php echo $mp_langpackage->mp_find;?>
        </h2>
        <div class="tabs">
            <ul class="menu">
                <li class="active">
        ...
        <?php }?>
        <?php foreach($search_rs as $rs){ //echo "<pre>";print_r($rs);exit;?>
            <div class="pals_list" onmouseover="this.className += ' pals_list_active';" onmouseout="this.className='pals_list';">
                <div class="right">
                    <p id='operate_<?php echo $rs["user_id"];?>'>
                        <a href='javascript:<?php echo str_replace("{uid}",$rs['user_id'],$send_join_js);?>'>
                            <?php echo "&nbsp;&nbsp;";?>
                            <?php echo str_replace( "{he}",get_TP_pals_sex($rs[ 'user_sex']),$mp_langpackage->mp_add_mypals);?>
                        </a>
                    </p>
                    <div class="content_ch1" style="background:url(skin/default/jooyea/images/ch.jpg) ">
                        ...
                        title="<?php echo $rf_langpackage->rf_liaotian;?>" onclick="top.i_im_talkWin('<?php echo $rs[user_id];?>','imWin');">
                        </a> -->
                        <a style="width:22px;height:22px;float:left;display:block" href="javascript:;"
                        title="<?php echo $rf_langpackage->rf_liaotian;?>" onclick="parent.open_chat('<?php echo $rs[user_id];?>','<?php echo $rs[user_name];?>','<?php echo $rs[user_ico];?>');">
                        </a>
                        <a style="width:22px;height:22px;float:left;display:block" href="javascript:;"
                        title="<?php echo $rf_langpackage->rf_dazhaohu;?>" onclick="top.hi_action(<?php echo $rs[user_id];?>)">
                        </a>
                        <a style="width:22px;height:22px;float:left;display:block" href="javascript:;"
                        title="<?php echo $rf_langpackage->rf_fayoujian;?>" onclick="top.frame_content.location.href='modules.php?app=msg_creator&2id=<?php echo $rs[user_id];?>';return false;">
                        </a>
                ...
                <div class="avatar">
                    <a href="home2.0.php?h=<?php echo $rs['user_id'];?>" target="_blank">
                        <img alt="<?php echo $mp_langpackage->mp_en_home;?>" title='<?php echo $mp_langpackage->mp_sex;?>:<?php echo get_pals_sex($rs['
                        user_sex ']);?>' src='<?php echo $rs["user_ico"];?>' />
                    </a>
                </div>
                <dl>
                    <dd>
                        <?php echo $mp_langpackage->mp_name;?>：
                        <?php echo filt_word($rs[ "user_name"]);?>
                    </dd>
                    <dd>
                        <?php echo $mp_langpackage->mp_reside;?>：
                        <?php echo get_pals_reside($rs[ 'reside_province'],$rs[ 'reside_city']);?>
                    </dd>
                    <dd>
                        <?php echo $mp_langpackage->mp_birth;?>：
                        <?php echo get_pals_birth($rs[ 'birth_province'],$rs[ 'birth_city']);?>
                    </dd>
                </dl>
            </div>
            <?php }?>
