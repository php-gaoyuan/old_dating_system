...
function CheckForm(){
	...
	function show_set_config(set_option,pid){
		...
		show_config.getInfo("modules2.0.php","GET","app","app=poll_show_config&set_option="+set_option+"&pid="+pid,function(c){show_config_callback(c);});
	}

	function act_config_callback(content,type_value){
		if(content=='success'){
			if(type_value=='del_poll'){
				window.location.href="modules2.0.php?app=poll_mine";
			}else{
...
<?php if($is_self=='Y'){?>
<div class="create_button"><a href="javascript:location.href='modules2.0.php?app=poll_send';"><?php echo $pol_langpackage->pol_send;?></a></div>
<?php }?>
...
<div class="tabs">
    ...
    <li class="<?php echo $mine_active;?>"><a href="modules2.0.php?app=poll_mine" hidefocus="true"><?php echo $pol_langpackage->pol_mine;?></a></li>
  </ul>
</div>
<?php }?>
    ...
    <?php if($ses_uid&&$poll_row['user_id']!=$ses_uid){?>
		...
		<div class="poll_initiator <?php echo $sendor_info;?>">
			<div class='poll_photo'>
				<a class="avatar" href="home2.0.php?h=<?php echo $poll_row['user_id'];?>" target="_blank">
			...
			<div class="poll_userinfo">
				...
				<p><a class="first" href='home2.0.php?h=<?php echo $poll_row['user_id'];?>' target='_blank'><?php echo str_replace("{name}",filt_word($poll_row['username']),$pol_langpackage->pol_who_home);?></a><a href="modules2.0.php?app=poll_list&user_id=<?php echo $poll_row['user_id'];?>">TA<?php echo $pol_langpackage->pol_vote;?></a><a href='javascript:window.history.go(-1);'><?php echo $pol_langpackage->pol_re_last;?></a></p>
			</div>
		</div>
		<div class="clear"></div>
