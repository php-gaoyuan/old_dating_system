...
//
    //验证30分钟体验时间
        ...
        if($nowtime>=$endtime || $user_info['user_sex'] ==1 ){
            ...
            if($user_info['user_group']=='base' || $user_info['user_group']=='1'){
                if($page_num>2){
                    echo "<script>alert('".$mp_langpackage->mp_quanxian."');location.href='modules2.0.php?app=user_pay';</script>";
                    exit();
                }
 ...
 //var_dump(page_show($isNull,$page_num,$page_total));exit;
   ...
   //$online_list=$dbo->getRs($sql);
    ...
    $sql="select user_id,user_name,user_ico from $t_users  limit 1";
    ...
    */
	...
	if(!$user['user_ico'] && !$_COOKIE['uico']){
		...
		//echo "<script>setTimeout(function(){window.location.href='/modules2.0.php?app=user_ico'},2000)</script>";
	}
	if($user['is_pass']==0){
		echo "<script>top.Dialog.alert('".$l_langpackage->l_lock_u."');</script>";
		echo "<script>window.location.href='/modules2.0.php?app=user_info';</script>";
	}
...
function list_recent_affair(h_id,ra_type,is_more,page){
	...
	if(ra_type=='' && ra_type!==0)	ra_type=jq('#affair_type').val();
		...
		if(ra_type!=5){
			...
			jq.get("modules2.0.php?app=recent_affair&",{'t':ra_type,'start_num':start_num},function(c){
				if(is_more==1 && c){
			...
			jq.get("modules2.0.php?app=recent_online&",{'page':page},function(c){
...
<!-- <div class="set_fresh_news"><a class="highlight" href="modules2.0.php?app=user_affair"><?php echo $rf_langpackage->rf_new_set;?></a></div> -->
