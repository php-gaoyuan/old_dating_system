        ...
        if($nowtime>=$endtime){
              ...
              if($type==1){//发件箱
                ...
                if($msg_row['state']=="0"){
                   ...
                   $reButUrl="do2.0.php?act=msg_send&to_id=$mess_id";
                }else{
                   ...
                   $reButUrl=$reTurnUrl;
                          ...
                          if($sex!=0){//非女性会员
                              ...
                              if(empty($recorder)){
                                  if($to_user_info['stamps_num']<1){  	  	  	
                                      echo "<script type='text/javascript'>alert('".$m_langpackage->m_stampsmsg."');location.href='modules2.0.php?app=user_stamps';</script>";
                                      exit;
                                  }else{
                    ...
                    $reButUrl="modules2.0.php?app=msg_creator&2id=$from_user_id&rt=".urlencode($mess_title)."&mesid=".$mess_id;
...
function mypals_add(other_id){
	...
	mypals_add.getInfo("do2.0.php","get","app","act=add_mypals&other_id="+other_id,function(c){mypals_add_callback(c,other_id);}); 
}

function join_event(event_id,fellow,template){
	var event_add=new Ajax();
	event_add.getInfo("do2.0.php?act=event_join","post","app","event_id="+event_id+"&fellow="+fellow+"&template="+template,function(c){parent.Dialog.alert(c);location.href="modules2.0.php?app=event_space&event_id="+event_id}); 
}
</script>
</head>
<body id="iframecontent">
    ...
    <div class="tabs" style="margin-top:0">
        <ul class="menu">
            ...
            <li <?php if($_GET['t']==0){echo 'class="active"';}?>><a href="modules2.0.php?app=msg_minbox" title="<?php echo $m_langpackage->m_in;?>" hidefocus="true"><?php echo $m_langpackage->m_in;?></a></li>
            <li <?php if($_GET['t']==1){echo 'class="active"';}?>><a href="modules2.0.php?app=msg_moutbox" title="<?php echo $m_langpackage->m_out;?>" hidefocus="true"><?php echo $m_langpackage->m_out;?></a></li>
	...
	<table class="form_table <?php echo $isset_data;?>" >
		...
		<tr>
			...
			<td>
				<input type=button class="regular-btn" value="<?php echo $m_langpackage->m_del;?>"	onclick='location.href="do2.0.php?act=msg_del&id=<?php echo $msg_id;?>&t=<?php echo get_argg("t");?>";'>
			</td>
		</tr>
	</table>
	</div>
