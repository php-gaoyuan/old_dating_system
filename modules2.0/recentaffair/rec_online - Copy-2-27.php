...
<li id="feed_<?php echo $rs['online_id'];?>">
	...
	<div class="details">
		...
		<?php if($rs['is_txrz']==1){echo '<a href="modules.php?app=renzheng"><i class="txrz_home" title="'.$u_langpackage->touxiangrenzheng.'"></i></a>';}?>
		<span style="position:relative;top:-4px"><span title="<?php echo count_level($rs['integral']);?>"><?php echo grade($rs['integral']);?></span></span>
			<div class="add_f content_ch1" style="background:url(skin/default/jooyea/images/ch.jpg) no-repreat;height:22px;width:90px;position:absolute;right:0;top:0;">
				...
				<a style="width:22px;height:22px;float:left;display:block" href="javascript:;" title="<?php echo $rf_langpackage->rf_fayoujian;?>" onclick="<?php if($userinfo['user_group']==2 || $userinfo['user_group'] == 3){ ?> top.frame_content.location.href='modules.php?app=msg_creator&2id=<?php echo $rs[user_id];?> ';return false; <?php } else { ?> alert('<?php echo $u_langpackage->readmore; ?>');<?php } ?>"></a>
...
<?php if($userinfo['user_group']==2 || $userinfo['user_group'] == 3){
	...
	<span><a href='http://www.henimeet.com/main.php?app=user_upgrade' target='_blank'>$u_langpackage->readmore</a></span>";
}
