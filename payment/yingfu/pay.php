...
$data = [
    ...
    'order_id'            => $orderId,
    //卡信息
    'card_number'           => aesEncryptHex($md5key,$card_number),
    'exp_year'              => aesEncryptHex($md5key,$exp_year),
    'exp_month'             => aesEncryptHex($md5key,$exp_month),
    'cvv'                   => aesEncryptHex($md5key,$cvv),
    //HASH,商户号+md5Key+订单号+订单金额+订单币种+网站url，按照顺序拼接，计算SHA-256摘要值，并转为16进制字符串（小写）
    'hash'              => sha256Encrypt($merchant_id.$md5key.$orderId.$total.$currency.$website),
    'version'       =>'20201001',
 ...
 * @return string
 */
function aesEncryptHex($md5key, $string)
{
