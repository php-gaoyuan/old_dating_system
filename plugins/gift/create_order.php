...
if($_POST && ((!empty($_POST['toself']) || !empty($_POST['tofriends']) ))){
	...
	if($accept_name){
		...
		if(empty($accept_info)){
			...
			echo "<script>top.location.href='http://www.pauzzz.com/main2.0.php?app=giftshop';</script>";
			exit;
		}
	}
...
<body>
	...
	<div  class="gift_li"><label><input type="radio" id="label1" name="towho" checked /><?php echo $pu_langpackage->songziji;?>
<input type="text" name="toself" id="toself" placeholder="请输入自己用户名"  />
	</label></div>
	<div  class="gift_li" style="border-bottom:1px dotted #ccc;padding-bottom:15px;">



	<label><input type="radio" id="label2" name="towho"  onclick="$('#toself').val('')"/><?php echo $pu_langpackage->songpengyou;?></label><input type="text" onfocus="$('#label2').click();$('#toself').val('')" id="tofriends" name="tofriends"  placeholder="my friend name" size="15"/>




	<select id="friends_list" name="friends_select" onchange="document.getElementById('tofriends').value=value;$('#toself').val('')">
	...
	<div class="giftorderinfo2">
		<ul>
			<li><a href="giftitem.php?id=<?php echo $id;?>"><img src="http://www.pauzzz.com/rootimg.php?src=<?php echo $gifts['yuanpatch'];?>&h=70&w=70&zc=1" /></a></li><li><?php echo $gifts['money'];?></li><li><?php echo $_REQUEST['num'];?></li><li><?php echo $gifts['money']*$_REQUEST['num'];?></li>
		</ul>
	</div>
