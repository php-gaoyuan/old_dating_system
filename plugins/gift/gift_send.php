...
$giftinfo=$dbo->getRow($sql);
$sql="select * from gift_news where  typeid='$type'";
$gifts=$dbo->getRs($sql);
...
#gift_page_list
{
	float:right;text-align:center;margin:10px 0 5px;
}
#gift_page_list a
{
...
<body onload="getGifts(<?php echo $giftinfo['typeid'];?>)">
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" style="color:#333333;">
  ...
  <tr>
	...
	<td>
		<ul id="zishu">
		  <li class="nav_a active"><a href="#" onclick="getGifts(<?php echo $giftinfo['typeid'];?>);changeStyle_gift(this);"><span><?php echo $gf_langpackage->gf_gift;?></span></a></li>
  ...
  <tr >
	...
	<td height="130"><textarea name="msg" style="border:1px #ccc solid;" cols="36" rows="5" id="textfield"></textarea></td>
  </tr>
</table>
<!--<div id="gift_friends">
...
function getGifts(type,index)
{
	//调用系统的Ajax类
	ajax=new Ajax(); 
	ajax.getInfo("/plugins/gift/gift_list2.php?id=<?php echo $id;?>","post","get","type="+type+"&index="+index,"gift_list"); 
}
/*//得到好友列表
function getFriends()
{
...
if($accept_name!="" && $gift!="")
{
	...
	if(($user_info['golds']-$score)>=0)
	{
		...
		$sql="insert into wy_balance set type='4',uid='$send_id',uname='$send_name',touid='$accept_id',touname='$accept_name',message='送礼物，价格：".$score."',state='2',addtime=now(),funds='$score',ordernumber='$ordernumber'";
		//echo $sql;exit;
