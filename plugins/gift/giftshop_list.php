...
if(!$userinfo['is_view_gift'] && $type==2){
    echo "<div class='giftbox'>$gl->gf_zanshimeiyou</div>";exit;
}
    ...
    if ($gifts) {
        foreach ($gifts as $gift) {
            if ($_COOKIE['lp_name'] == 'en') {
                $gift['giftname'] = $gift['giftname_en'];
            }
            $yuanpatchs = explode("|", $gift['yuanpatch']);
            echo "<div class='giftbox'><a href=giftitem.php?id=" . $gift['id'] . " target='_self'><img class='gift_img' title='" . $gift['giftname'] . "' src='/rootimg.php?src=/" . $yuanpatchs[0] . "&h=130&w=130&zc=1'/><span>" . $gift['giftname'] . "</span><span style='color:#c20606;font-weight:700'>" . $gift['money'] . "<img src='/skin/gaoyuan/images/svg/gold.svg' style='vertical-align:middle;width:15px;'/></span></a></div>";
        }
    } else {
