...
Dialog.prototype.create = function () {
    ...
    var html = '\
  <table id="_DialogTable_' + this.ID + '" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" onselectstart="return false;" style="-moz-user-select: none; font-size:12px; border:3px solid #eee; line-height:1.4;">\
    <tr id="_Draghandle_' + this.ID + '" style="' + (this.Drag ? "cursor: move;" : "") + '">\
      <td height="40" style="background:#D15A0D;"><div style="padding: 5px 0 0 16px; float: left; font-weight: bold; font-size:13px; color:#fff; "><span id="_Title_' + this.ID + '">' + this.Title + '</span></div>\
        ...
        return "";
          ...
          <tr id="_ButtonRow_' + this.ID + '" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            <td height="36"><div id="_DialogButtons_' + this.ID + '" style="border-top: 1px solid #DADEE5; padding: 8px 20px; text-align: right; background-color:#f6f6f6;">\
                <input type="button" style="background:url(servtools/dialog/images/button.gif)  no-repeat;width:58px;height:24px;line-height:24px;cursor:pointer;overflow:hidden;border:0;" value="OK" id="_ButtonOK_' + this.ID + '"/>\
                <input type="button" style="background:url(servtools/dialog/images/button.gif)  no-repeat;width:58px;height:24px;margin-left:8px;cursor:pointer;line-height:24px;overflow:hidden;border:0;" value="Cancel" onclick="Dialog.getInstance(\'' + this.ID + '\').close();" id="_ButtonCancel_' + this.ID + '"/>\
              </div></td>\
          </tr>\
        </table></td>\
    </tr>\
...
Dialog.alert2 = function (msg, func, w, h) {
    var w = w || 550,
        h = h || 110;
    var diag = new Dialog({
        Width: w,
    ...
    diag.ShowButtonRow = true;
    diag.Title = "详细说明(Detailed description)";
    diag.CancelEvent = function () {
        diag.close();
        if (func) func();
    };
...
Dialog.confirm = function (msg, funcOK, funcCal, w, h) {
    ...
    diag.ShowButtonRow = true;
    diag.Title = "Information confirm";
    diag.CancelEvent = function () {
        diag.close();
        if (funcCal) {
            funcCal();
