...
function toggle(targetid,type_id,mod_id){
	...
	if(target.style.display==""){
		$("openreply_"+type_id+"_"+mod_id).innerHTML = "Reply";
		target.style.display="none";
	}else{
		$("openreply_"+type_id+"_"+mod_id).innerHTML = "Pack up";
...
function act_report_callback(content){
	if(content=="true"){
		Dialog.alert('report success');
	}else{
		Dialog.alert(content);
	}
}
function like_callback(content,k){
		frame_content.$('likes_num'+k).innerHTML=content;
		
	
}
function act_share_callback(content,share_type){
  if(content=="success"){
  	Dialog.alert("share success");
