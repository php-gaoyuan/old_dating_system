...
<script type="text/javascript">
    layui.use(["layim", "layer", "form"], function () {
        ...
        layim.on('tool(video)', function (insert, send, obj) { //事件中的tool为固定字符，而code则为过滤器，对应的是工具别名（alias）
            if (chat.userinfo.user_sex == '0' && chat.userinfo.user_group < 3) {
                //弹出升级提示
                ...
                // },2000);
                return false;
            } else if (chat.userinfo.user_sex == '1' && chat.userinfo.user_group < 4) {
                //弹出升级提示
                layer.msg("Your level does not match, Please upgrade.");
                return false;
            } else {
        ...
        layim.on('chatChange', function (res) {
            //判断权限
            if (chat.userinfo.user_group < 3) {
                $(".layui-show .layim-chat-textarea textarea").attr("disabled", true);
                //layer.msg("您不是VIP用户不能使用即时聊天，请充值升级");
                layer.msg("You are not a VIP user, you can not use instant chat, please recharge upgrade.");
                setTimeout(function () {
                    window.location.href = "/main2.0.php?app=user_upgrade";
                }, 2000);
                return false;
            ...
            $(".layui-show .tr_select").change(function () {
                ...
                if (val == "0") {
                    ...
                    if (txt.trim() == "") {
                        ...
                        if (chat.userinfo.user_sex > 0) {
                            layer.confirm("翻译需要扣除金币", {icon: 3, title: '提示'}, function (index) {
                                $.get("fanyi.php", {fid: chat.userinfo.user_id, lan: txt, tos: val}, function (res) {
                                    if (res == 0) {
                                        layer.msg("Lack of gold coin");
                                        return false;
                                    }
